...
public class ParameterManager(IParameterRepository parameterRepository) : DomainService
{
	...
	{
		if (parameter != null && string.Equals( parameter.Code , code , StringComparison.OrdinalIgnoreCase ))
			return;
		if (await parameterRepository.IsExistCodeAsync( code ))
			throw new BusinessException( "存在相同的参数编码" ).WithData( "Code" , code );
	}
}